...
- `/fugm_beat [意图]`：让时悠主动推进当前场景一拍，不消耗玩家行动。
- `/fugm_heartbeat [force|强制]`：手动触发当前团心跳检查；需要时会让时悠主动推进或提醒当前行动者。
...
时悠会自行选择查看、生成或强制重画工具。只有渲染器实际返回图片后，她才会说地图已经画好；新增地点和绘图写在同一句时，会先保存地点再开始生成。
...
插件会把心跳判断交给 FU-GM HTTP 服务的 `/v1/session/heartbeat`。它默认比较克制：
...
- `enable_idle_monitor`：是否启用后台 idle monitor，默认 true。
- `idle_monitor_auto_reply`：是否允许后台心跳主动发消息，默认 true。
