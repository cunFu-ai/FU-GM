...
def _extract_line_item(clause: str) -> str:
    ...
    patterns = [
        ...
        r"(?:我|我们)?\s*(?:不希望|不想|不愿意|不接受|不能接受|接受不了|受不了|希望不要|请不要|不要|别(?![人的])|禁止)\s*(?:在游戏中|在游戏里|游戏中|游戏里|故事里|剧情里)?\s*(?:出现|有|包含|涉及|提到)?\s*(?P<item>[^，,。！？；;\n]+)",
...
def _extract_veil_item(clause: str) -> str:
    ...
    patterns = [
        r"(?:我|我们)?\s*(?:不希望|不想|不愿意|希望不要|请不要|不要|别(?![人的]))\s*(?:明确|直接|详细|细致|正面|过度)?\s*(?:描写|描述|展开|聚焦|细写|细说)\s*(?P<item>[^，,。！？；;\n]+)",
