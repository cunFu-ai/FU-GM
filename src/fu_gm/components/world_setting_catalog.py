    ...
    def _normalized_name(
        ...
        *,
        ...
        if category in self.PUBLIC_SCALARS:
            return ""
        if category in self.PUBLIC_LISTS or category in self.PRIVATE_LISTS:
            # List entries are identified by their complete committed text so
            # later updates and deletes can target the exact statement.  The
            # entity-name limit must not truncate valid long histories,
            # mysteries or threats; their normal value limit still applies.
            return self._require_value(name if existing else value)
        return self._require_name(name)

    @classmethod
    def canonical_mutation_arguments(
        cls,
        operation: str,
        arguments: dict[str, object],
    ) -> dict[str, object]:
        """Return arguments in the shape the catalog can persist losslessly.

        List entries have no separate durable title: their complete ``value``
        is also their identity.  Proposal confirmation packets are generated by
        Python rather than authored by the model, so normalize those packets
        before signing them instead of issuing an impossible ``name != value``
        follow-up.  Direct model calls remain strictly validated by the world
        setting tool and do not silently discard a supplied title.
        """
        ...
        if (
            action == "create"
            and category in (cls.PUBLIC_LISTS | cls.PRIVATE_LISTS)
        ):
